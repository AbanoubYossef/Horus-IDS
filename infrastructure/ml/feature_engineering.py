...
def engineer_features(df, feat_cols):
    ...
    new_features["bwd_fwd_byte_ratio"]  = bwd_bytes / fwd_bytes
    ...
    new_features["rst_flag_ratio"]     = rst_count / total_pkts
    new_features["header_payload_ratio"] = col("Fwd Header Length") / (fwd_bytes + eps)
    new_features["active_idle_ratio"]  = col("Active Mean") / (col("Idle Mean") + eps)
